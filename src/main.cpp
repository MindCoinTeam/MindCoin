...
uint256 hashGenesisBlock("0x000000dc2faeae8d4ed692d10e58d2953f1e2947db7d02865a5d886f2a071918"); //mainnet
 ...
 int64 static GetBlockValue(int nBits, int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 32000 blocks
    nSubsidy >>= (nHeight / 32000); // MindCoin 32,000

    return nSubsidy + nFees;
}


static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // mindcoin: 1 Day
static const int64 nTargetSpacing = 120; // mindcoin: 2 Minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if ((pindexLast->nHeight+1) % nInterval != 0)
    {
        ...
        if (fTestNet)
        {
            // If the new block's timestamp is more than 2* 31 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
                ...
                if (pindexLast->nHeight+1 >= 100) { DiffMode = 4; }
                //else if (pindexLast->nHeight+1 >= 25) { DiffMode = 3; }
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        if(fTestNet) {
            ...
            // Check proof of work (Here in case of architecture issues with DGW v3)
            if (nBits != GetNextWorkRequired(pindexPrev, this))
                    return state.DoS(100, error("AcceptBlock() : incorrect proof of work"));
        }
...
{
    // mindcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000a827136126244e45a649253a557767130d676106bb46b1788c428920afa");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        const char* pszTimestamp = "Teach mental health in primary schools, advisory group says";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 15 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04CADEA2C160FA8A649869A1C78C4A7BF762972329F53A57AA5E3BCE065BA16BB3DACFAECB7A6ADB6A2CDEB7DBDB6B60EF1851A694240C76B88BB3D531B62B6412") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1451493189;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 108140;

        if (fTestNet)
        {
            block.nTime    = 1451267809;
            block.nNonce   = 1662268;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xc0a86ea3f705dc86bcc20d6482e8f74d932d002db090c07c716c9afe928afaf6"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // mindcoin: increase each by adding 2 to bitcoin's value.
